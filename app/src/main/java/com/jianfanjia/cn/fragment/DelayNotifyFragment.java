...
/**
 * @author fengliang
 * @ClassName: DelayNotifyFragment
 * @Description: 改期提醒
 * @date 2015-8-26 下午1:09:52
 */
public class DelayNotifyFragment extends BaseFragment implements ApiUiUpdateListener, DelayInfoListener, PullToRefreshBase.OnRefreshListener2<RecyclerView> {
    private static final String TAG = DelayNotifyFragment.class.getName();
    private PullToRefreshRecycleView delayListView = null;
    ...
    public void initView(View view) {
        ...
        delayListView = (PullToRefreshRecycleView) view.findViewById(R.id.tip_delay__listview);
    ...
    public void setUserVisibleHint(boolean isVisibleToUser) {
        ...
        if (isVisibleToUser) {
            // fragment可见时加载数据
            LogTool.d(TAG, "DelayNotifyFragment 可见");
            getRescheduleNotifyList();
        } else {
            // 不可见时不执行操作
            LogTool.d(TAG, "DelayNotifyFragment 不可见");
        }
    }

    @Override
    public void setListener() {
        delayListView.setOnRefreshListener(this);
    ...
    public void onPullDownToRefresh(PullToRefreshBase<RecyclerView> refreshView) {
        getRescheduleNotifyList();
        delayListView.onRefreshComplete();
    }
    ...
    public void loadSuccess(Object data) {
        ...
        if (null != delayList && delayList.size() > 0) {
            ...
            delayListView.setAdapter(delayAdapter);
            delayListView.setVisibility(View.VISIBLE);
            emptyLayout.setVisibility(View.GONE);
            errorLayout.setVisibility(View.GONE);
        } else {
            delayListView.setVisibility(View.GONE);
