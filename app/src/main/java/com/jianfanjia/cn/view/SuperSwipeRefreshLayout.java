...
public class SuperSwipeRefreshLayout extends ViewGroup {
    ...
    protected void onLayout(boolean changed, int left, int top, int right,
        ...
        LogTool.d(LOG_TAG, "debug:onLayout childHeight = " + childHeight);
        ...
        switch (action) {
            ...
            case MotionEvent.ACTION_MOVE:
                if (mActivePointerId == INVALID_POINTER) {
                    LogTool.d(LOG_TAG, "Got ACTION_MOVE event but don't have an active pointer id.");
                    return false;
                }
    ...
    private boolean handlerPushTouchEvent(MotionEvent ev, int action) {
        switch (action) {
            case MotionEvent.ACTION_DOWN:
                mActivePointerId = MotionEventCompat.getPointerId(ev, 0);
                mIsBeingDragged = false;
                LogTool.d(LOG_TAG, "debug:onTouchEvent ACTION_DOWN");
                break;
            case MotionEvent.ACTION_MOVE: {
                final int pointerIndex = MotionEventCompat.findPointerIndex(ev,
                        mActivePointerId);
